...
export const seedSupraA80 = {
  ...
  engines: [
    '2JZ-GE 3.0L naturally aspirated DOHC inline-six (~220 hp / 217 hp)',
    '2JZ-GTE 3.0L sequential twin-turbo DOHC inline-six (JDM 276 hp gentleman’s-agreement rating; US 320 hp; Europe ~325 hp)',
  ],
  ...
  notableTrims: [
    ...
    { name: 'RZ / RZ-S (JDM)', note: 'Top JDM twin-turbo trims; RZ paired the 2JZ-GTE with the Getrag V161 6-speed. 276 hp gentleman’s-agreement rating.' },
    { name: 'SZ / SZ-R (JDM)', note: 'Naturally aspirated 2JZ-GE JDM trims; SZ-R got a 6-speed manual.' },
    { name: 'Turbo (Europe)', note: 'Europe received only the twin-turbo car, rated ~325 hp in most references - a separate ECU/calibration from the US.' },
  ],
  specs: {
    ...
    'Power (JDM Turbo)': '276 hp (gentleman’s-agreement cap; widely believed underrated)',
    'Power (NA)': '~220 hp / 217 hp',
    ...
    'Top speed': 'Electronically limited to ~155 mph (≈155 mph)',
...
The design brief was "less is more." The A80 was shorter, lower and wider than the car it replaced, and Toyota leaned hard on weight reduction - even specifying hollow carpet fibers and aluminum components - to shed about 220 lb versus the A70. The styling, with its long hood and available tall rear wing, owed more to the legendary 2000GT than to its immediate predecessor.
...
**Rust.** Check the hatch/liftback lip, rear quarters and arches, sills, and the spare-wheel well / trunk floor for soft spots - especially on cars stored outdoors. Sound metal there usually means a sound shell.
...
**Rust.** Hatch lip, rear arches/quarters, sills and the trunk/spare-well floor are the watch areas, particularly on outdoor-stored cars.
...
**Cooling and ancillaries.** Radiator, intercooler and related cooling components wear with age and matter more once a car is driven hard or modified.`,
  ...
  sources: [
    {
      ...
      reliability: 'high',
      notes:
        'Manufacturer heritage history. Source for development under Isao Tsuzuki, 1993 Chicago debut, ~220 lb weight savings, JZ-series engine intro, 276 hp Japanese voluntary limit, first six-speed gearbox, 155/156 mph limiter, July 2002 end-of-production and the emissions rationale.',
    },
    {
      ref: 'wikipedia',
    ...
    },
    {
      ...
      reliability: 'medium',
      notes:
        'Spec/market aggregator: 2JZ-GE 220, 2JZ-GTE 276 hp JDM / 320 hp US / ~325 hp EU; V160 (US/EU) vs V161 (JDM RZ); condition risks (synchro wear, sequential-turbo faults, rust); stock cars as the auction benchmark.',
    },
    {
      ref: 'jza80-de',
      ...
      reliability: 'medium',
      notes:
        'Marque-club technical reference on why export 2JZ-GTE engines were rated higher than the JDM 276 hp car (turbo internals, injectors and calibration differences).',
    },
    {
      ref: 'classic-com',
  ...
  claims: [
    ...
    },
    {
      section: 'specs',
      claimText:
        'The 2JZ-GTE was rated 276 hp in Japan, 320 hp in the US, and roughly 325 hp in Europe - the same basic engine with different turbos, injectors and calibration per market.',
      confidence: 'high',
      status: 'disputed',
      sourceRefs: ['toyota-uk', 'jdmbuysell', 'jza80-de', 'wikipedia'],
      conflictNote:
        'The JDM 276 hp figure reflects the Japanese makers’ "gentleman’s agreement" power cap, not a true measured ceiling - independent dyno tests routinely show JDM cars making more, and many sources state the JDM and export engines are far closer in real output than the 280-vs-320 spread implies. Export cars (US 320 hp / EU ~325 hp) used steel-wheel turbos, larger injectors and different ECU mapping. We present the published per-market ratings and flag that the JDM 276 hp is widely regarded as an artificially capped number, not actual peak output.',
    },
    {
      section: 'specs',
      claimText:
    ...
    },
    {
      section: 'problems',
      claimText:
        'The 2JZ-GTE bottom end (cast-iron block, forged crank, sedan/motorsport-derived design) is over-engineered relative to its factory power rating, giving it large tuning headroom; well-built engines reportedly make four-figure horsepower and extreme drag examples have been quoted past 2,000 hp.',
      confidence: 'high',
      status: 'disputed',
      sourceRefs: ['toyota-uk', 'classicmotorsports', 'jdmbuysell'],
      conflictNote:
        'The bottom end’s strength is well established. The headline power claims (e.g. Toyota UK’s "up to 2041 hp") describe extreme, heavily-modified one-off builds, not a guaranteed ceiling for any given engine - and real-world reliable headroom on stock internals is debated (commonly cited around ~700-800 hp with supporting mods). We flag the eye-catching numbers as outliers and frame the headroom as exceptional-but-build-dependent.',
    },
    {
      section: 'problems',
      claimText:
        'Common A80 issues include sequential twin-turbo faults (boost creep / transition problems from worn actuators), V160 second/third synchro wear, aging ECU/electrical components, and structural rust at the hatch lip, rear arches, sills and trunk floor.',
      confidence: 'high',
      status: 'verified',
    ...
    },
    {
      section: 'market',
      claimText:
        'Mk4 Supra values surged from around 2021 (Hagerty noted roughly a 40 percent rise in six months after a landmark mid-2021 auction), reaching six figures for the best original US Turbo 6-speed cars by the mid-2020s, helped by the 2JZ tuning legend and the Fast & Furious halo.',
      confidence: 'medium',
      status: 'unverified',
