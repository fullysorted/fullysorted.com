...
The living restriction is mileage. Under 49 CFR 591.6(f)(2) the importer must state that until the vehicle "is not less than 25 years old", they will not sell it, transfer possession or title, license it or operate it on public roads except on terms NHTSA authorizes; must hold an insurance policy conditioned on the vehicle not accumulating "more than 2,500 miles in any 12-month period"; must allow NHTSA to inspect the vehicle at any time to verify that; and must state that the vehicle "will not be used on the public roads unless it is in compliance with the regulations of the Environmental Protection Agency." Breach is not a paperwork matter: 49 CFR 591.7(d) provides that violating a term of the authorization, "including a failure to allow inspection upon request to verify that the accumulated mileage of the vehicle is not more than 2,500 miles in any 12-month period, shall be considered a violation of 49 U.S.C. 30112(a) for which a civil penalty may be imposed", and "will also act to void the authorization and require the exportation of the vehicle."

The difference from the 25-year route is therefore total. The 25-year route is a declaration made at the border and the car is then an ordinary used car in federal terms. Show or Display is a license with a locked title, a mileage cap, an inspection right and an export remedy attached, and it still has to satisfy the EPA separately.`,
    keyPoints: [
...
Sellers advertise cars as "eligible next year" on model year alone. Before committing money, get a photograph of the build plate and match the month to the month of the intended entry.`,
    keyPoints: [
      ...
      'Build plate first; NHTSA accepts first-sale invoices, 25-year-old registration documents, or a recognized historical society statement as fallback evidence',
...
export const IMPORT_DISCLAIMER =
  'This page is general information about United States vehicle import law, compiled on 22 August 2026 from the statutes, regulations, agency forms and agency guidance cited in the sources list. It is not legal advice, it is not customs or tax advice, and it is not a substitute for the current text of the law. Federal rules, tariff classifications and state registration requirements change, agency practice does not always track the regulatory text exactly, and the correct treatment of any particular vehicle depends on facts that only an inspection of that vehicle and its documents can establish: its date of manufacture, its configuration, its classification and the state it will be registered in. Verify the position for your own vehicle with NHTSA, the EPA Imports Hotline, U.S. Customs and Border Protection, your state titling agency, a licensed customs broker or a qualified attorney before committing money or shipping anything.';

export const IMPORT_DISCLAIMER_SHORT =
  'General information, not legal, customs or tax advice, compiled on 22 August 2026; verify current requirements with NHTSA, EPA, CBP, your state titling agency or a qualified professional before acting.';
