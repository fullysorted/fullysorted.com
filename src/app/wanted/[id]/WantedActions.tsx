...
export function WantedActions({ id, mine, open, pending, handle, replyCount }: {
  ...
  async function close(action: "found" | "remove") {
    if (action === "remove" && !window.confirm("Take this post down?")) return;
    setState("sending");
    const res = await fetch(`/api/wanted/${id}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
    setState("idle");
  ...
  if (mine) {
    ...
    return (
      <div className="mt-8 rounded-2xl p-5" style={{ background: "var(--bg-surface)" }}>
        <p className="font-semibold" style={{ color: INK }}>Your post{open ? ` · ${replyCount} ${replyCount === 1 ? "reply" : "replies"} so far` : ""}</p>
        <p className="text-sm mt-1" style={{ color: MUTED }}>Replies arrive in your email. Close the post when you are done so people stop looking.</p>
        <div className="flex flex-wrap gap-2 mt-4">
          {open && (
            <button onClick={() => close("found")} disabled={state === "sending"} className="px-5 py-3 rounded-full text-sm font-bold text-white disabled:opacity-60" style={{ background: TEAL }}>
              Found it
            </button>
          )}
  ...
  if (state === "sent") {
    return (
      <div className="mt-8 rounded-2xl p-6" style={{ background: "var(--bg-surface)" }}>
        ...
        <p className="text-sm mt-1" style={{ color: MUTED }}>{handle ? `@${handle}` : "The poster"} has your message and your email, and will write back if it is a fit.</p>
      </div>
    );
  }
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        setError(null);
        try {
          const res = await fetch(`/api/wanted/${id}/reply`, {
    ...
    >
      <p className="font-display text-xl" style={{ color: INK }}>Know where one is?</p>
      <p className="text-sm -mt-1" style={{ color: MUTED }}>Your message and email go to {handle ? `@${handle}` : "the poster"} only. Nothing is posted publicly.</p>
      <textarea required minLength={10} maxLength={3000} rows={4} value={message} onChange={(e) => setMessage(e.target.value)} aria-label="Your message" placeholder="What you have or know, and where it is" className={field} />
      ...
      <button type="submit" disabled={state === "sending"} className="h-12 rounded-full text-[15px] font-bold text-white disabled:opacity-60" style={{ background: TEAL }}>
        {state === "sending" ? "Sending..." : "Send to the poster"}
      </button>
      {error && (
