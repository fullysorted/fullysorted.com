...
import { ensurePartsTables, PARTS_DAYS } from '@/lib/parts';
import { sendPartsApproved } from '@/lib/email';

/**
 * Admin moderation for the Parts board.
 * GET  ?status=pending|open|closed     listings with their owner and reply count
 * POST { action: 'approve' | 'reject' | 'remove' | 'reopen', id, note? }
 * Approving starts the 90-day clock and emails the seller once.
 */
  ...
  try {
    ...
    const [summary] = await sql`
      SELECT
        ...
        (SELECT COUNT(*)::int FROM parts_replies) AS replies
      FROM parts_posts
    `;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'approve' || action === 'reopen') {
      ...
      const emailed = await sendPartsApproved({ to: String(done[0].email), id: rowId, title: String(done[0].title) }).catch(() => false);
