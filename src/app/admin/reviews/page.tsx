...
const TABS = ["pending", "published", "invited", "expired", "rejected", "all"] as const;
            ...
            <li key={r.id} className="rounded-xl border border-stone-200 bg-white p-5">
              ...
              {r.status === "invited" || r.status === "expired" ? (
                <p className="text-sm italic text-stone-500">
                  Invited {r.invited_at ? new Date(r.invited_at).toLocaleDateString() : ""} —{" "}
                  {r.status === "expired" ? "expired unanswered. Sending a fresh invite reopens this row." : "not written yet."}
                </p>
              ) : (
              ...
              {replyDraft?.id === r.id ? (
                ...
                <div className="flex flex-wrap items-center gap-3 mt-4 pt-3 border-t border-stone-100">
                  {r.status !== "published" && r.status !== "invited" && r.status !== "expired" && (
                    <button
                  ...
                  {r.status !== "rejected" && r.status !== "invited" && r.status !== "expired" && (
                    <button
                      onClick={() => reject(r.id)}
                      disabled={busyId === r.id}
                  ...
                  {(r.status === "invited" || r.status === "expired") && (
                    ...
                    >
                      <Trash2 className="w-3.5 h-3.5" /> Delete this unanswered invite
                    </button>
                  )}
                </div>
              )}
